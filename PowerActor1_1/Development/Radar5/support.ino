...
void broadcastCatAlarm(targetData target, byte sensorNr){
  broadcastMsg(catObserved, target, sensorNr);
}
void broadcastHB() {
  targetData dummy;
  dummy.geschw = HBPeriode;
  broadcastMsg(HB, dummy, 0);
}
void broadcastMsg(byte typ, targetData target, byte sensorNr) {
udpMsgStruct udpMsg;
  time_t now;
  time(&now);
  udpMsg.msgCode = typ;
  ...
  udpMsg.sender = broadcastId+sensorNr;
