...
// 2 - BLE Radar einbau
// 3 - Div. Tests
// 4 - "Stand" Sender, Broadcast , telnet, non blocking BLE connect
// 5 - Anpassung an Udp record
#include <FastLED.h>
#include <esp_wifi.h>
#include <esp_now.h>
#include <WiFi.h>
...
Task allLedOffAfter1000(HBPeriode,2,&oneTimeLedOffHelper);
void sendHB();
...
void loop() {
  ...
  if (newBleData) {
    newBleData = false;
    for (int i=0;i < 3; i++) {
      if (BLEtarget[i].active) broadcastCatAlarm(BLEtarget[i],i);
    }
      if (BLEtarget[0].active) led("R",0);
      else led("",0);
  }
