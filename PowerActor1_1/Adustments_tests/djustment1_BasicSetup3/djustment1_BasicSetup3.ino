// 1 - 1/16 step bei tasteDruck + Led wenn Reed Kontakt - no Time
...
//    rotor.digitalWrite(STEP, LOW);
    ...
    Serial << globalPosition << "   " << winkel.getCumulativePosition() << "  "  << zz  << endl;
    zz++;
    ArduinoOTA.handle();
    taskMgr.execute();
  }
...
void loop() {
  if (!digitalRead(taster)) {
    rotorStep(right,sixteenthStep);
    Serial << globalPosition << "   " << winkel.getCumulativePosition() << endl;
    while (!digitalRead(taster));
  }
  if (leftReed()) led("R",0); else led("",0);
  if (rightReed()) led("R",1); else led("",1);
  delay(150);
  ArduinoOTA.handle();
  taskMgr.execute();
}
