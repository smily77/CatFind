...
void handleCommand(const xMsg& m) {
  // cmdReboot fuer die CatCam bewusst SPERREN (vor handleCommonMsg abfangen):
  // ein Soft-Reset (ESP.restart) kappt die Stromversorgung des Vision-Moduls nicht,
  // es haengt danach bis zum physischen Power-Cycle. Deshalb kein Remote-Reset hier.
  {
    cmdPayload pre;
    if (m.header.msgCode == commandMsg && getPayload(m, pre) && pre.cmd == cmdReboot) {
      sendUdpTextln("CatCam: Remote-Reset gesperrt (Vision-Modul braucht Power-Cycle)");
      return;
    }
  }
